...
if not os.path.isfile("AfterImage/AfterImage.c"): #has not yet been compiled, so try to do so...
    cmd = "python AfterImage/setup.py build_ext --inplace"
    subprocess.call(cmd,shell=True)
#Import dependencies
import AfterImage.netStat as ns
import csv
import numpy as np
...
class FE:
    ...
    def __prep__(self):
        ...
        ##If file is pcap
            ...
            if os.path.isfile("D:\\Wireshark\\tshark.exe"):
    ...
    def get_next_vector(self):
        ...
        ### Extract Features
        try:
            result =  self.nstat.updateGetStats(IPtype, srcMAC, dstMAC, srcIP, srcproto, dstIP, dstproto,
                                                 int(framelen),
                                                 float(timestamp))
            return result
    ...
    def pcap2tsv_with_tshark(self):
        ...
        cmd =  '''"D:\\Wireshark\\tshark" -r '''+ self.path +''' -T fields '''+ fields +''' -E header=y -E occurrence=f > '''+self.path+".tsv"
        subprocess.call(cmd,shell=True)
